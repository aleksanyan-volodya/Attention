...
def explain_prediction(
    ...
    Uses gradient × input (saliency): the gradient of the predicted class score
    with respect to each token embedding, multiplied element-wise by the embedding
    itself, then summed across the embedding dimension. This gives one importance
    score per token — no model architecture changes needed.
    ...
    input_ids = processed_text.unsqueeze(0).to(device)  # (1, max_length)

    # Hook to capture the embedding output tensor and keep its gradient.
    # (Intermediate tensors lose their grad after backward by default.)
    captured = {}
    ...
    # Gradient × input: how much each embedding dimension is actively contributing.
    # Sum across d_model and take absolute value → one scalar per token position.
    ...
    # Normalize to [0, 1] so scores are easy to read
    if scores.max() > 0:
